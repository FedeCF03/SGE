...
/*
    Cada trámite está asociado a un expediente por medio del ExpedienteId
    ...
    un usuario podría tener permiso para eliminar trámites sin tenerlo para eliminar
    expedientes. Sin embargo, se asume que aquellos usuarios con permisos para eliminar expedientes
    también tendrán la capacidad para eliminar trámites. La razón es que al dar de baja un expediente, se
    deben eliminar también todos los trámites asociados.
Al ingresar un trámite:
...
Descripción general
El objetivo es desarrollar un sistema de gestión de expedientes que posibilite la organización y
administración efectiva de la información. Cada expediente contendrá un conjunto de datos propios, junto
con una colección de trámites asociados.
Entidades
Se trabajará con dos entidades fundamentales: Expediente y Trámite. Ambos elementos serán
gestionados y persistidos de forma independiente, siguiendo el patrón de repositorios e inyección de
dependencias visto en la teoría 7 de este curso.
Cada expediente y trámite estará caracterizado por una propiedad Id única, lo que facilitará su
identificación entre otras instancias del mismo tipo. Para mantener la separación en la persistencia de
expedientes y trámites, será esencial establecer una relación que permita determinar qué trámites
pertenecen a qué expedientes. A tal efecto, se empleará una propiedad denominada ExpedienteId en la
clase Trámite, destinada a asociar un trámite a un expediente específico.
...
● Notificación
● Pase al Archivo
Se asume, por simplicidad, que estos estados y etiquetas son fijos y no requerirán modificaciones, por lo
que se pueden codificar en un tipo enumerativo.
Permisos de usuario
El sistema también gestiona usuarios y sus respectivos permisos. Esto se debe a la necesidad de que los
usuarios cuenten con los permisos adecuados para llevar a cabo las operaciones requeridas. Aunque la
administración de usuarios (alta, baja y modificación) se pospondrá para la próxima entrega, es importante
destacar que todas las operaciones relacionadas con las entidades Trámite y Expediente deben llevar
consigo la información del Id de usuario que las está ejecutando.
Todos los usuarios tendrán acceso de lectura sobre ambas entidades (Expediente y Trámite). No obstante,
para realizar altas, bajas o modificaciones será necesario contar con permisos explícitos. Cada usuario
podrá poseer varios de los permisos que se detallan a continuación:
...
A modo de ejemplo, un usuario podría tener permiso para eliminar trámites sin tenerlo para eliminar
expedientes. Sin embargo, se asume que aquellos usuarios con permisos para eliminar expedientes
también tendrán la capacidad para eliminar trámites. La razón es que al dar de baja un expediente, se
deben eliminar también todos los trámites asociados.
Validaciones
Además, el sistema debe realizar validaciones para evitar el ingreso de expedientes o trámites que no
cumplan con ciertos requisitos. Estos requisitos son los siguientes:
● La carátula de un expediente no puede estar vacía
● El contenido de un trámite no puede estar vacío
● El id de usuario (en trámites y expedientes) debe ser un id válido (entero mayor que 0)
Cambio automático del estado de un expediente
El sistema también debe gestionar ciertos desencadenantes que se activan al manipular los trámites de los
expedientes, lo que ocasionará un cambio automático en el estado del expediente. Al agregar un trámite a
un expediente, dependiendo de la etiqueta de dicho trámite, puede ser necesario realizar un cambio de
estado automático en el expediente asociado.
...
estado "Finalizado".
Este cambio automático del estado del expediente también debe llevarse a cabo al eliminar un trámite. Si
como resultado de esta eliminación, el último trámite restante es uno de los mencionados anteriormente,
que ocasiona un cambio automático de estado, dicho cambio también deberá realizarse automáticamente.
Lo mismo ocurrirá si se modifica el último trámite, cambiando su etiqueta por otra que también
desencadene un cambio automático en el estado del expediente.
Manejo de excepciones
El sistema debe estar preparado para manejar al menos tres nuevas excepciones relacionadas con la
autorización de usuarios, la validación de entidades y los repositorios.
La excepción AutorizacionException debe ser lanzada cuando un usuario intenta realizar una operación
...
La excepción RepositorioException debe ser lanzada cuando la entidad que se intenta eliminar, modificar
o acceder no existe en el repositorio correspondiente.
Detalles de implementación
Codificar la solución llamada SGE (por Sistema de Gestion de Expedientes) con tres proyectos:
SGE.Aplicacion, SGE.Repositorios y SGE.Consola, utilizando como modelo de arquitectura limpia el visto
en la teoría 7 de este curso.
Proyecto SGE.Aplicacion
En el proyecto SGE.Aplicacion codificar las 2 entidades mencionadas: Expediente y Tramite. Puede
resultar de utilidad Invalidar el método ToString() en ambas entidades.
Para obtener un código desacoplado, implementar las validaciones de ambas entidades por medio de
clases especializadas. Así, para validar la entidad Tramite definir una clase TramiteValidador. Para validar
la entidad Expediente definir la clase ExpedienteValidador.
Codificar también en este proyecto los enumerativos EstadoExpediente y EtiquetaTramite. Estos
enumerativos contendrán los valores que podrán asignarse a las propiedades Estado (de la entidad
Expediente) y Etiqueta (de la entidad Trámite), respectivamente. Además, implementar el enumerativo
Permiso, el cual incluirá los valores correspondientes a los permisos que el sistema debe manejar.
Codificar también en este proyecto un servicio de autorización para validar las credenciales del usuario al
momento de realizar operaciones. El servicio debe implementar la siguiente interfaz:
public interface IServicioAutorizacion
{
bool PoseeElPermiso(int IdUsuario, Permiso permiso);
}
Para esta primera entrega, desarrollar un servicio de autorización provisional llamado
ServicioAutorizacionProvisorio, que implemente la interfaz IServicioAutorizacion. Este servicio
